    ...
    menu {
        canvas (title "canvas", type "node", icon "icon.drop.clear") {
            fill (title "fill", type "node", icon "icon.speed.png") {
                ...
                zero (title "Fill Zero", type "pad", value 0..1, icon "icon.drop.clear")
                // fillZero
                one (title "Fill Ones", type "pad", value 0..1, icon "icon.drop.gray")
                // fillOne
            }
            ...
            color (title "color", type "node", icon "icon.pal.main.png") {
                fade (title "Screen Scroll", type "rectXY", x 0..1, y 0..1, icon "icon.scroll.png")
                // scrollBox
                tilt (title "Brush Tilt", type "toggle", value 0..1, icon "icon.pen.tilt.png")
                // brushTilt
            }
        }
        ...
        brush (title "brush", type "node", icon "icon.cell.brush.png") {
            size (title "brush size", type "slider", value 0..1, icon "icon.pen.press.png")
            // brushSize
            press (title "pen press", type "toggle", value 0..1, icon "icon.pen.press.png")
            // brushPress
            tilt (title "pen title", type "toggle", value 0..1, icon "icon.pen.tilt.png")
            // brushPress
        }
        ...
        camera (title "camera", type "node", icon "icon.camera.png") {
            ...
            face (title "facing", type "toggle", value 0..1, icon "icon.camera.flip.png")
            >> sky.shader.cellCamera.flip xfade (title "cross fade", type "slider", value 0..1, icon "icon.pearl.white.png")
